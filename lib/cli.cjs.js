...
class RtdCssCliCompiler {
    compile(inputFilePath, outputDirectoryPath) {
        ...
        const createFileOptionsWithDevice = (outputDeviceName) => {
            return {
                compileOptions: {
                    compileOnlyThisDevice: outputDeviceName,
                },
                filePath: this.createOutputFilePath(outputDirectoryPath, parsedInputFilePath.name, parsedInputFilePath.ext, outputDeviceName),
            };
        };
        const outFileOptionsList = [
        ...
        for (const outFileOptions of outFileOptionsList) {
            const outputRoot = compiler.compile(inputRoot, outFileOptions.compileOptions, new PostcssCssDriver());
    ...
    createOutputFilePath(outputDirectoryPath, outputFileNameWithoutExt, outputExt, outputDeviceName = null) {
        const result = path.join(outputDirectoryPath, [
...
RtdCssCliCompiler.cssNamespaceExt = '.rtd';
...
class CompileCommand extends Command {
    ...
    validateAndGetInputData(argv) {
        if (!(CommandUtils.validateOptionListNotEmpty(this.program, argv, [this.inputCssFileOption, this.outputDirectoryOption])
    ...
    actionBody(inputData) {
        const compiler = new RtdCssCliCompiler();
