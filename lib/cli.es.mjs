...
(function (DataSchema) {
    ...
    class SchemaFactory {
        createValueOptionEx(optionOrSubOption, valueFormat, valueType, name, optionToProperty, required, transformFunc = null) {
            let optionSchema;
            ...
            if (optionOrSubOption) {
                optionSchema = new OptionSchema(name, bindingToData, OptionType.Single, valueSchema, required, null, transformFunc);
            }
            else {
                optionSchema = new SubOptionSchema(name, bindingToData, OptionType.Single, valueSchema, required, transformFunc);
            }
            return optionSchema;
        }
        createStringOption(name, optionToProperty, required, transformFunc = null) {
            return (this.createValueOptionEx(true, ValueFormat.SingleValue, ValueType.String, name, optionToProperty, required, transformFunc));
        }
        createStringArrayOption(name, optionToProperty, required, transformFunc = null) {
            return (this.createValueOptionEx(true, ValueFormat.MultipleValue, ValueType.String, name, optionToProperty, required, transformFunc));
        }
        createStringSubOption(name, optionToProperty, required, transformFunc = null) {
            return (this.createValueOptionEx(false, ValueFormat.SingleValue, ValueType.String, name, optionToProperty, required, transformFunc));
        }
        createStringArraySubOption(name, optionToProperty, required, transformFunc = null) {
            return (this.createValueOptionEx(false, ValueFormat.MultipleValue, ValueType.String, name, optionToProperty, required, transformFunc));
        }
        createFloatOption(name, optionToProperty, required, transformFunc = null) {
            return (this.createValueOptionEx(true, ValueFormat.SingleValue, ValueType.Float, name, optionToProperty, required, transformFunc));
        }
        createFloatSubOption(name, optionToProperty, required, transformFunc = null) {
            return (this.createValueOptionEx(false, ValueFormat.SingleValue, ValueType.Float, name, optionToProperty, required, transformFunc));
        }
...
(function (DeviceExpandingInputConfigModule) {
    ...
    (function (ConfigParser) {
        ...
        ConfigParser.parse = parse;
    })(ConfigParser = DeviceExpandingInputConfigModule.ConfigParser || (DeviceExpandingInputConfigModule.ConfigParser = {}));
    ...
    class ConfigExporter {
        ...
        exportToScriptConfig() {
            const scriptConfig = {
                breakpointsForDeviceByDevice: (DictionaryCreator.createObjectFromArray(this._deviceList, device => device.type, device => {
                    const breakpoints = ConfigExporter.parseBreakpoints(device.breakpoints, this._deviceByName, this._initialBreakpointByName);
                    return new DeviceExpandingScriptConfigModule.BreakpointsForDevice(ConfigExporter.inputDeviceToScriptDevice(device), breakpoints);
                })),
            };
            scriptConfig.breakpointsForDeviceByDevice[DeviceType.Unknown] = new DeviceExpandingScriptConfigModule.BreakpointsForDevice(this._config.unknownDevice, this._initialBreakpoints);
            return scriptConfig;
        }
        static createDeviceListFromConfig(config) {
            const allNullableDevices = [
                config.mobileDevice,
                config.tabletDevice,
        ...
        static createBreakpointByName(breakpoints) {
            return DictionaryCreator.createDictionaryFromArray(breakpoints, breakpoint => breakpoint.device.name, breakpoint => breakpoint);
        }
        static parseBreakpointsRaw(tokens, deviceByName) {
            ...
            for (let i = 0; i < tokens.length; i += 2) {
                ...
                // Breakpoint
                const curBreakpoint = new DeviceExpandingScriptConfigModule.Breakpoint(ConfigExporter.inputDeviceToScriptDevice(device), maxWidth);
        ...
        static validateBreakpoints_validOrdering(breakpoints, all = false) {
            ...
            const deviceOrderingValid = all
                ...
                : ArrayUtils.isEqual(deviceTypes, ConfigExporter.allOrderedDeviceTypes.filter(type => deviceTypes.includes(type)));
        ...
        static inputDeviceToScriptDevice(inputDevice) {
            const scriptDevice = new DeviceExpandingScriptConfigModule.Device(inputDevice.type, inputDevice.name, inputDevice.cssClass);
            return scriptDevice;
        }
    }
    ConfigExporter.allOrderedDeviceTypes = [
        DeviceType.Mobile,
        DeviceType.Tablet,
        DeviceType.Desktop,
    ];
...
class CssCompiler {
    compile(inputCss, options = null, cssDriver = defaultCssDriver) {
        const inCssRoot = this.inputCssToCssRoot(inputCss, cssDriver);
...
(function (BrowserScript) {
    BrowserScript.rtdScript = rtdScript;
    BrowserScript.uaParserScript = uaParserScript;
...
class FileCompiler {
    ...
    compile(outputRequest) {
        ...
        if (outputRequest.css) {
            ...
            for (const outFileOptions of outFileOptionsList) {
                const outputResult = cssCompiler.compile(inputRoot, outFileOptions.compileOptions, this._cssDriver);
        ...
        if (outputRequest.js) {
            ...
            for (const outFileOptions of outFileOptionsList) {
                ...
                if (!outFileOptions.device) {
                    ...
                    scriptSb.add('\n\n');
                    scriptSb.add(BrowserScript.rtdScript
                    ...
                    const breakpointsForDeviceJson = TextFormatter.jsonStringifyPretty(breakpointsForDevice, TextFormatter.indentTab())
                        .addTabsAtBeginOfAllLines(numberOfTabsForScriptVars)
                        .getText()
                        .trim();
                    scriptSb.add(BrowserScript.rtdScript
    ...
    createOutputFilePath(outputExt, outputDeviceName = null) {
        ...
        const result = path.join(outputDirectoryPath, [outputFileNameWithoutExt, '.rtd', outputDeviceName ? `.${outputDeviceName}` : '', outputExt].join(''));
        return result;
    }
    ...
    createOutputJsFileOptionsList(outputRequest, config) {
        ...
        if (outputRequest.js === FileCompiler.OutputRequestJs.General) {
            ...
            const deviceList = [
                null,
                ...DictionaryUtils.getValues(config.breakpointsForDeviceByDevice).map(breakpointsForDevice => breakpointsForDevice.device),
            ];
...
(function (FileCompiler) {
    let OutputRequestCss;
    (function (OutputRequestCss) {
        OutputRequestCss["General"] = "General";
        OutputRequestCss["GeneralAndDevices"] = "GeneralAndDevices";
    })(OutputRequestCss = FileCompiler.OutputRequestCss || (FileCompiler.OutputRequestCss = {}));
    let OutputRequestJs;
    (function (OutputRequestJs) {
        OutputRequestJs["General"] = "General";
        OutputRequestJs["GeneralAndDevices"] = "GeneralAndDevices";
    })(OutputRequestJs = FileCompiler.OutputRequestJs || (FileCompiler.OutputRequestJs = {}));
...
class CompileCommand extends Command {
    ...
    validateAndGetInputData(argv) {
        if (!CommandUtils.validateOptionListNotEmpty(this.program, argv, [
        ...
        try {
            ...
            this.program.printer.showCommandHelpWithSingleError(this.outputDirectoryOption, `Can not create directory "${inputData.outputDirectoryPath}"`);
            return;
        }
        if (!(CommandUtils.validateFileExists(this.program, argv, this.inputCssFileOption) &&
    ...
    actionBody(inputData) {
        const compiler = new FileCompiler(inputData.inputFilePath, inputData.outputDirectoryPath, new PostcssCssDriver());
        compiler.compile({
