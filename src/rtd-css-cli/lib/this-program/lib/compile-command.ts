import { Cli } from '../../cli';
import { RtdCssCliCompiler } from '../../rtd-css-cli-compiler';
...
export class CompileCommand extends Cli.Command<CompileCommandInputData> {
	...
	constructor(program: Cli.Program) {
		...
		this.inputCssFileOption = new Cli.CommandOption(
			this.program,
			{
				name: 'input-css-file',
				alias: 'i',
		...
		this.outputDirectoryOption = new Cli.CommandOption(
			this.program,
			{
	...
	protected validateAndGetInputData(argv: Cli.CliArgv): CompileCommandInputData {
		if (
			!(
				Cli.CommandUtils.validateOptionListNotEmpty(this.program, argv, [this.inputCssFileOption, this.outputDirectoryOption])
